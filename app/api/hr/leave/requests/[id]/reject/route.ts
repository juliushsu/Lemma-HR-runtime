...
export async function POST(request: Request, { params }: Params) {
  const schema_version = "hr.leave.request.reject.v1";
  ...
  const { data, error } = await ctx.supabase.rpc("reject_leave_request", {
    p_leave_request_id: id,
    p_approver_user_id: ctx.user_id,
    p_rejection_reason: reason
  });

  if (error) return map_leave_rpc_error(schema_version, error, "Failed to reject leave request");
